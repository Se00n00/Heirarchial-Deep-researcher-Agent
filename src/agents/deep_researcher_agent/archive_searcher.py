'''
This code defines an asynchronous tool named ArchiveSearcherTool that searches the Wayback Machine for archived versions of 
a given URL closest to a specified date. It uses the requests library to fetch data from the Wayback Machine API and another 
tool, WebFetcherTool, to retrieve and format the content of the archived page. The tool returns the title and content of the 
archived page or an error message if no archive is found.

We need to try and test this separately. to know if this works just fine.
