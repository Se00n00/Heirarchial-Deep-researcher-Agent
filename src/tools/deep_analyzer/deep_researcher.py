'''
this code implements a DeepResearcherTool that performs comprehensive research on a given query by 
conducting multi-level web searches and analyzing the content to extract key insights. It uses an 
LLM to optimize search queries, extract insights, and generate follow-up queries. The tool maintains 
a research context to track progress, including visited URLs and discovered insights, and produces 
a structured summary of findings with source attribution and relevance ratings.

we need to try this sepearately. to test if it works fine.
'''
