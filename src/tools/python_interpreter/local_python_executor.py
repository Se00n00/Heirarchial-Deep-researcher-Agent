# Copyright 2025 Heirarchical Agent --------- just a name for now
...
# List of Python built-in modules that are safe to use
...
def custom_print(*args, **kwargs):
    """Custom print function that captures output in the state dictionary."""
    # Get the state from function parameters if available
    state = kwargs.pop('_state', None)
    if state is None:
        # Try to get state from the call stack if not provided
        ...
        while frame:
            ...
            print("Warning: No state found for custom_print")
            return None

    # Convert all arguments to strings and join them
    output = " ".join(str(arg) for arg in args)
    
    # Initialize _print_outputs if not present
    if "_print_outputs" not in state:
        state["_print_outputs"] = ""
    
    # Add the output with a newline
    state["_print_outputs"] += output + "\n"
    return None
...
def evaluate_python_code(
    ...
    """
    Evaluates Python code in a safe environment with restricted imports and functions.
    
    Args:
        code: The Python code to evaluate
        static_tools: Dictionary of allowed functions
        custom_tools: Dictionary of custom tools
        state: Dictionary to store state (including print outputs)
        authorized_imports: List of modules that can be imported
        max_print_outputs_length: Maximum length of captured print outputs
    """
